...
class MotorControlUI:
    ...
    def update_motor_position(self, motor, value):
        try:
            ...
            motor_positions = {m: float(v.get()) for m, v in self.motor_values.items()}
            self.hand._set_motor_pos(motor_positions)
            # tk.Scale 的 command 回调中 value 是字符串，需要转换为 float
            value_float = float(value) if isinstance(value, str) else value
            print(f"Updated motor {motor} to position: {value_float:.1f}")
