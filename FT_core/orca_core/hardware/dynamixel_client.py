...
class DynamixelClient:
    ...
    def read_pos_vel_cur(self, skip_current: bool = False) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        Args:
            skip_current: Accepted for interface parity with HLClient. Dynamixel
                reads pos/vel/cur in a single bulk packet, so there is no extra
                cost; this flag is ignored.
        """
