...
from .hardware.hl_client import HLClient
...
class OrcaHand:
    ...
    def __init__(self, model_path: str = None, port: str = None):
        ...
        Args:
            ...
            port (str): Optional serial port override (e.g. 'COM7'). If None, uses config.yaml's 'port'.
        """
        ...
        self.port: str = port if port is not None else config.get('port', '/dev/ttyUSB0')
        ...
        self.client_type: str = config.get('client_type', 'dynamixel')
    ...
    def connect(self) -> tuple:
        """Connect to the hand with the configured client (DynamixelClient or HLClient).

        Returns:
            tuple[bool, str]: (Success status, message).
        """
        try:
            if self.client_type.lower() in ['hl', 'feitech', 'hletech']:
                self._dxl_client = HLClient(self.motor_ids, self.port, self.baudrate)
            else:
    ...
    def get_motor_pos(self, as_dict: bool = False) -> Union[np.ndarray, dict]:
        ...
        """
        with self._motor_lock:
            # skip_current=True：遥操热路径只读位置，跳过飞特 17 次逐电机电流读，
            # 显著降低单帧串口延迟（电流只在标定/张紧时由 get_motor_current 读取）。
    ...
    def _compute_wrap_offsets_dict(self):
        ...
        for i, motor_id in enumerate(self.motor_ids):
            limits = self.motor_limits_dict[motor_id]
            if any(l is None for l in limits):
                offsets[motor_id] = 0.0
                continue
            # Use min/max so this works for both normal and inverted joints.
            # Inverted joints (negative sign in joint_to_motor_map) store
            # limits with limits[0] > limits[1]; assuming limits[0]<limits[1]
            # here would mis-detect ±1-turn wrap and corrupt the joint angle.
            lo, hi = min(limits), max(limits)

            if motor_pos[i] < lo - 0.25 * np.pi: # Some buffer to compensate for noise/slack differences
                print(f"Motor ID {motor_id} is out of bounds: {lo} < {motor_pos[i]} < {hi}")
                offsets[motor_id] = -2 * np.pi

            elif motor_pos[i] > hi + 0.25 * np.pi: # Some buffer to compensate for noise/slack differences
                print(f"Motor ID {motor_id} is out of bounds: {lo} < {motor_pos[i]} < {hi}")
                offsets[motor_id] = +2 * np.pi
    ...
    def tension(self, move_motors: bool = False, blocking: bool = True, continuous: bool = False):
        if blocking:
            self._tension(move_motors, continuous=continuous)
        else:
            self._start_task(self._tension, move_motors, continuous=continuous)

    def _tension(self, move_motors: bool = False, continuous: bool = False):
        """Freeze the motors, so that the hand can be manually tensioned.

        Args:
            move_motors (bool): If True, the hand will move all motors positively for 3 seconds to set some initial tension.
            continuous (bool): If True, keep winding the motors positively until stopped
                (Ctrl+C in blocking mode, or stop_task() otherwise), then hold position.
                Lets you dial in tendon tension manually instead of the fixed 3-second move.
        """
        self.set_control_mode('current_based_position')
        if move_motors or continuous:
            motors_to_move = [
            ...
            if continuous:
                print("Continuously tensioning motors. Press Ctrl+C to stop winding and hold.")
                try:
...
class MockOrcaHand(OrcaHand):
    ...
    def connect(self) -> tuple:
        """Connects to the mock Dynamixel client (or mock HL client if configured).
...
if __name__ == "__main__":
    # Example usage:
    hand = OrcaHand()
    print(f"Client type: {hand.client_type}")
    print(f"Port: {hand.port}")
    print(f"Motor IDs: {hand.motor_ids}")
